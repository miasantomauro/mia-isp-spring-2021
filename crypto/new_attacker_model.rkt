...
pred wellformed {
  ...
  all d: Datum | all t: Timeslot | all a: Agent | d->t in a.learned_times iff {
    ...
    {{some m: Message | {d in m.data and t = m.recvTime and m.receiver = a}}
    or 
  ...
  all m: Message | m.data in ((m.sender).learned_times).(Timeslot - (m.sendTime).^tick) 
...
pred exploit_search {
  ...
  some m: Message | 
  {
    ...
    na in Attacker.learned_times.Timeslot 
